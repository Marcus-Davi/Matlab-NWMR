...
Tsim = 47.2;   %90      % tempo de simulação
h = 0.1;           % integrating time
D = 0.4;

x0 = [0;0;-0.2];  % x,y,theta referenciais para início de trajetória. 
X = x0;     %rungeKutta  

xa = [0;0;-0.2];  % x,y,theta referenciais para início de trajetória. 
X1 = xa;         %Euler
...
for k = 1:round(Tsim/h);  % 1 a tempo de simulação/ tempo de integração   
       ...
       if (k<=150);   %419
        w(k) = 0.025;  % variação da velocidade angular positiva
       elseif (k>=150)&&(k<=270);        
        w(k) = 0.30; 
       elseif (k>=270)&&(k<=360);        
        w(k) = -0.14; 
       else %if (k>=70)&&(k<=90);    
        w(k) = 0.287; % variação da velocidade angular negativa 
       end
