...
Tsim = 80;   %90      % tempo de simulação
h = 0.1;           % integrating time

xa = [0;0;0.0];  % x,y,theta referenciais para início de trajetória. 
X1 = xa;         %Euler
...
for k = 1:round(Tsim/h);  % 1 a tempo de simulação/ tempo de integração   
     ...
     %   x0(3)= 0;
        v(k) = 0.10; %= 0.3;
        w(k) = 0;  % variação da velocidade angular positiva
     %  elseif (k>=220)&&(k<=250);
     %   v(k) = 0.00; %= 0.3;
       % w(k) = 0.523; 
      elseif (k>=lado/(Ts*h))&&(k<=2*lado/(Ts*h));
          ...
          %x0(3)= pi/2;
        v(k) = 0.10; %= 0.3;
        w(k) = 0.0;  % variação da velocidade angular positiva
     % elseif (k>=470)&&(k<=500);
      %  v(k) = 0.00; %= 0.3;
      %  w(k) = 0.523;
     ...
     %   elseif (k>=720)&&(k<=750);
      ...
      %  w(k) = 0.523;
       elseif (k>=3*lado/(Ts*h))&&(k<=4*lado/(Ts*h));
        ...
        %  x0(3)= 3*pi/2;
        v(k) = 0.10; %= 0.3;
        w(k) = 0.00; % variação da velocidade angular negativa 
       else
           v(k)=0;
           w(k)=0;
       end
...
for k = 1:round(Tsim/h)-1;    % 1 a tempo de simulação/ tempo de integração  
...
plot(X(1,:),X(2,:),'r')
%legend('rungeKutta','euler','Trajetória Simulada')
xlabel('x[m]')
ylabel('y[m]')
